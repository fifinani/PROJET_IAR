...
IARRobotWorldModel::IARRobotWorldModel() : _cameraSensors ( boost::extents[12][7] )
{
    _initSensor = false;
    ...
    _energyLevel_A = gEnergyInit;
    _energyLevel_B = gEnergyInit;
...
void IARRobotWorldModel::initCameraSensors ( int nbSensors )
{
    if ( _initSensor == true )
    {
...
void IARRobotWorldModel::updateLandmarkSensor( int __landmarkId )
{
    if ( __landmarkId > gNbOfLandmarks )
    {
        std::cout << "[ERROR] Landmark " << __landmarkId << " does not exist in IARRobotWorldModel::updateLandmarkSensor(landmarkId). Exiting." << std::endl;
        exit (-1);
    }
...
void IARRobotWorldModel::updateLandmarkSensor()
{
    if ( gNbOfLandmarks == 0 )
    {
        std::cout << "[ERROR] There is no landmark to consider in IARRobotWorldModel::updateLandmarkSensor(). Exiting." << std::endl;
        exit (-1);
    }
